...
export const ModuleCard = memo(function ModuleCard({
  ...
  return (
    <section
      ref={rootRef}
      className={`canvas-module${framed ? ' with-frame' : ''}${selected ? ' selected' : ''}${settingsOpen ? ' editing' : ''}${dragging ? ' dragging' : ''}`}
      ...
      <Space className="canvas-module-actions" size="small">
        {handleRef && (
          <Button
            ...
            type="text"
            size="small"
            className="canvas-module-handle"
            icon={<HolderOutlined />}
            title="Drag to reorder (keyboard: Space, arrows, Space)"
            aria-label={`Reorder module ${module.title || index + 1}`}
        ...
        <Button
          ...
          onClick={(event) => {
            ...
            store.getState().moveModule(moduleId, index - 1);
          }}
        />
        <Button
          type="text"
